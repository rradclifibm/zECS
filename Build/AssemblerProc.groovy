...
// copy program to PDS
println("Copying ${properties.sourceDir}/$file to $asmPDS($member)")
new CopyToPDS().file(new File("${properties.sourceDir}/$file")).dataset(asmPDS).member(member).execute()
...
copybookGen.dd(new DDStatement().name("SYSIN").dsn("$asmPDS($member)").options("shr").report(true))
...
compile.dd(new DDStatement().name("SYSIN").dsn("$asmPDS($member)").options("shr"))
